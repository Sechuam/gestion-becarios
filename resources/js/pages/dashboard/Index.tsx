...
const DEFAULT_SECTIONS = ['row_metrics', 'row_top_charts', 'row_bottom_panels', 'row_alerts'];
...
function SortableRow({ id, children, isEditing }: { id: string; children: React.ReactNode; isEditing: boolean }) {
...
export default function Dashboard({
    ...
    const [bottomPanels, setBottomPanels] = useState<string[]>(DEFAULT_BOTTOM_PANELS);
    const [visibleWidgets, setVisibleWidgets] = useState<string[]>(DEFAULT_TOP_CHARTS.concat(DEFAULT_BOTTOM_PANELS).concat(['metrics', 'alerts']));
    ...
    const saveLayout = (newSections: string[], newTop: string[], newBottom: string[], newVisible: string[]) => {
        localStorage.setItem('dashboard-puzzle', JSON.stringify({
    ...
    const metrics: DashboardMetric[] = [
        { label: role === 'intern' ? 'Mi práctica' : 'Becarios activos', value: stats.active_interns, hint: role === 'admin' ? 'En todos los centros' : 'Dentro de tu alcance', icon: Users },
        { label: 'Tareas abiertas', value: stats.active_tasks, hint: 'Pendientes, activas o en revisión', icon: KanbanSquare },
        { label: 'Evaluaciones pendientes', value: stats.pending_evaluations, hint: 'Sin evaluación registrada este mes', icon: ClipboardCheck },
        { label: 'Cumplimiento horario', value: `${stats.attendance_compliance}%`, hint: 'Horas registradas sobre objetivo', icon: Clock3 },
        { label: 'Próximas finalizaciones', value: stats.upcoming_endings, hint: 'Prácticas que terminan en 30 días', icon: CalendarClock },
        { label: 'Alertas activas', value: stats.alerts, hint: 'Ausencias y jornadas por revisar', icon: AlertTriangle },
    ];

    const renderWidget = (id: string, isOverlay = false) => {
        switch (id) {
            case 'metrics': return <DashboardMetricCards metrics={metrics} />;
            case 'interns_chart': return <InternsByCenterChart data={interns_by_center} />;
            case 'task_chart': return <TaskStatusChart data={task_status_chart} />;
            case 'attendance': return (
                <div className="flex h-full flex-col gap-2.5">
                    <AttendanceChart className="flex-1" data={attendance_chart} />
                    <AttendanceStatsCard className="flex-1" completeAttendanceRate={stats.complete_attendance_rate} averageDelayMinutes={stats.average_delay_minutes} absenceRate={stats.absence_rate} />
                </div>
            );
            case 'agenda': return <TodayAgendaPanel className="h-full" todayAgenda={today_agenda} currentLog={current_log} />;
            case 'progress': return <InternTaskProgressPanel className="h-full" taskProgress={task_progress} averageResolutionDays={stats.average_task_resolution_days} />;
            case 'alerts': return <DashboardAlertCards alerts={alerts} />;
            default: return null;
        }
    };
    ...
    const renderSection = (rowId: string) => {
        switch (rowId) {
            case 'row_metrics':
                ...
                return (
                    <SortableRow key={rowId} id={rowId} isEditing={isEditing}>
                        <DashboardWidgetWrapper id="metrics" isEditing={isEditing} className="w-full">
                            {renderWidget('metrics')}
                        </DashboardWidgetWrapper>
                    </SortableRow>
                );
            case 'row_top_charts':
                const visibleTop = topCharts.filter(id => visibleWidgets.includes(id));
                if (visibleTop.length === 0) return null;
                return (
                    <SortableRow key={rowId} id={rowId} isEditing={isEditing}>
                        <SortableContext items={visibleTop} strategy={rectSortingStrategy}>
                            <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                                {topCharts.map(id => {
                                    if (!visibleWidgets.includes(id)) return null;
                                    return (
                                        <DashboardWidgetWrapper 
                                            key={id} id={id} isEditing={isEditing} 
                                            className={id === 'interns_chart' ? "md:col-span-7" : "md:col-span-5"}
                                        >
                                            {renderWidget(id)}
                                        </DashboardWidgetWrapper>
                                    );
            ...
            case 'row_bottom_panels':
                const visibleBottom = bottomPanels.filter(id => visibleWidgets.includes(id));
                if (visibleBottom.length === 0) return null;
                return (
                    <SortableRow key={rowId} id={rowId} isEditing={isEditing}>
                        <SortableContext items={visibleBottom} strategy={rectSortingStrategy}>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                {bottomPanels.map(id => {
                                    if (!visibleWidgets.includes(id)) return null;
                                    return (
                                        <DashboardWidgetWrapper key={id} id={id} isEditing={isEditing} className="col-span-1">
                                            {renderWidget(id)}
                                        </DashboardWidgetWrapper>
                                    );
                                })}
            ...
            case 'row_alerts':
                ...
                return (
                    <SortableRow key={rowId} id={rowId} isEditing={isEditing}>
                        <DashboardWidgetWrapper id="alerts" isEditing={isEditing} className="w-full">
                            {renderWidget('alerts')}
                        </DashboardWidgetWrapper>
                    </SortableRow>
                );
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="space-y-4">
                ...
                <DndContext sensors={sensors} collisionDetection={rectIntersection} onDragStart={handleDragStart} onDragEnd={handleDragEnd} onDragCancel={() => setActiveId(null)}>
                    ...
                    <DragOverlay dropAnimation={{ sideEffects: defaultDropAnimationSideEffects({ styles: { active: { opacity: '0.4' } } }) }}>
                        {activeId ? (
                            <div className="opacity-90 shadow-2xl scale-[1.02] transition-transform pointer-events-none">
                                {sections.includes(activeId) ? (
                                    <div className="bg-white/50 backdrop-blur-sm p-4 rounded-3xl border-2 border-dashed border-sidebar/20">
